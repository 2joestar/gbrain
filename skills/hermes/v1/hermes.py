...
class Hermes:
    ...
    def _cfg_val(self, key: str, default):
        if not self._cfg:
            self._cfg = load_cfg("system").get("hermes", {})
    ...
    def check_hallucination(self, text: str) -> bool:
        ...
        for p in filler_patterns:
            if re.search(p, t):
                log("hermes_hallucination", {"snippet": text[:100]})
                return True
        return False
    ...
    def validate_memory_write(self, content: str) -> bool:
        ...
        if len(content) > 100_000:
            log("hermes_memory_oversized", {"size": len(content)})
            return False
        return True
    ...
    def validate(self, action: dict) -> bool:
        ...
        if risk == "high":
            if action.get("approved") is True:
                log("hermes_high_risk_approved", {"type": atype})
                self._approved += 1
                return True
            log("hermes_high_risk_blocked", {"type": atype})
            self._blocked += 1
            return False
